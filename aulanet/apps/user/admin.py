...
class UserAdmin(UserAdminDjango):
    fieldsets = UserAdminDjango.fieldsets + (
        (None, {'fields': ('id', 'city', 'school','type', 'birthdate', 'avatar', 'related_school')}),
    ) 

    add_fieldsets = (
        (None, {
            'fields': ('username', 'email', 'password1', 'password2', 'city', 'school','type', 'birthdate', 'avatar', 'related_school')}),
    )
...
def add_to_Admin(self, request, queryset):
    ...
    self.message_user(request, "Usuarios añadidos al grupo 'Administradores' correctamente.")
...
def remove_from_Admin(self, request, queryset):
    ...
    self.message_user(request, "Usuarios eliminados del grupo 'Administradores' correctamente.")
remove_from_Admin.short_description = "Eliminar del grupo 'Administradores'"


actions = [add_to_Registered, add_to_Contributor, add_to_Admin, remove_from_Registered, remove_from_Contributor, remove_from_Admin]
